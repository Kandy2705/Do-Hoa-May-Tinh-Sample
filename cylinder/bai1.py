...
class OrbitViewer:
    ...
    def run(self):
        ...
        while not glfw.window_should_close(self.win):
            ...
            projection = T.perspective(45, w/h, 0.1, 100.0)

            eye = np.array([6.0, self.height, 0.0])
            target = np.array([0, 0, 0])
            up = np.array([0, 1, 0])
            
            view = T.lookat(eye=eye, target=target, up=up)
...
def main():
    ...
    # 2. SỬA LỖI TẠI ĐÂY: Thay 'self' bằng 'viewer'
    viewer.radius = 6.0
    viewer.height = 3.0
    
    # 3. Gán model vào viewer để hàm run() có thể tìm thấy self.model
    viewer.model = model 
    
    # Nếu ông có danh sách drawables để xử lý phím bấm:
    viewer.drawables.append(model)
